...
function game_menu:on_started()
    ...
    if tier then
        local title = sol.language.get_string('menu.Continued')
        title = title:gsub('X', tier)
        table.insert(self.items, { surface = create_surface(title), action = resume_action })
    end
    table.insert(self.items, { surface = create_surface(sol.language.get_string('menu.NewGame')), action = new_game_action })
    table.insert(self.items, { surface = create_surface(sol.language.get_string('menu.Settings')), action = help_action })
    table.insert(self.items, { surface = create_surface(sol.language.get_string('menu.Exit')), action = sol.main.exit })
    self.current_item = 1
